...
function Home() {
  ...
  const createTodo = async () => {
    console.log('In create-todo function');
    try {
      if(!newTodo)
        return;
  ...
  <div className="mt-4 bg-gray-100 max-w-lg lg:max-w-xl rounded-lg shadow-lg mx-8 sm:mx-auto p-6">
    ...
    <div className="flex mb-4">
      ...
      <button className="bg-blue-600 border rounded-r-md text-white px-4 py-2 hover:bg-blue-900 duration-300" onClick={createTodo}>Add</button>
    </div>
    <ul className="space-y-2">
      {todos.map((todo, index) => (
                      ...
                      <div className="flex items-center">
                        ...
                        <input type="checkbox" className="mr-2" checked={todo.isComplete} onChange={() => updateTodoStatus(todo._id)}/>
                        <span className="text-gray-500 font-semibold">{todo.text}</span>
                      </div>
                      <button className="text-red-500 hover:text-red-800 duration-300">Delete</button>
                    </li>
      ))}
    </ul>
    <p className="mt-4 text-center text-sm text-gray-800  ">0 Todos remaining</p>
    <button className="mt-6 px-4 py-2 bg-red-500 text-white rounded-md hover:bg-red-800 duration-300 mx-auto block">Logout</button>
  </div>
  )
}
